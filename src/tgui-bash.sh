...
if [ $# -ge 1 ] && [ "$1" = "-h" ]; then
  if [ "$2" = 0 ]; then
    docpath="@NEOTERM_PREFIX@/share/tgui-bash/manual"
  fi
  if [ "$2" = 1 ]; then
    docpath="@NEOTERM_PREFIX@/share/tgui-bash/tutorial"
  fi
  if [ -z "$docpath" ]; then
    while true; do
  ...
  if [ "$4" = 0 ]; then
    @NEOTERM_PREFIX@/share/tgui-bash/docviewer.sh "$docpath$docpathext"
    exit 0
  fi
  if [ "$4" = 1 ]; then
  ...
  if [ -z "$browser" ]; then
    while true; do
      ...
      echo "0: The NeoTerm:GUI WebView"
      echo "1: Android integrated HTML Viewer"
      echo "2: Chrome"
      echo "3: Firefox"
      read -r resp
      if [ "$resp" == 0 ]; then
        @NEOTERM_PREFIX@/share/tgui-bash/manual.html "$docpath$docpathext"
        exit 0
      fi
      if [ "$resp" == 1 ]; then
        browser="com.android.htmlviewer/.HTMLViewerActivity"
...
# find helper binary
helper="@NEOTERM_PREFIX@/libexec/neoterm-gui-bash-helper"
...
# contact plugin
# use neoterm-am if available
if ! neoterm-am broadcast -n io.neoterm.gui/.GUIReceiver --es mainSocket "$sock_main" --es eventSocket "$sock_event" >/dev/null 2>&1; then
  am broadcast -n io.neoterm.gui/.GUIReceiver --es mainSocket "$sock_main" --es eventSocket "$sock_event" >/dev/null 2>&1
fi
